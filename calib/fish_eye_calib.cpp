#include "fish_eye_calib.hpp"
#include <iostream>
#include <fstream>
#include <sstream>

FishEyeStereoCalibration::FishEyeStereoCalibration()
    : m_flags(cv::fisheye::CALIB_RECOMPUTE_EXTRINSIC | cv::fisheye::CALIB_FIX_SKEW | cv::fisheye::CALIB_CHECK_COND)
...
void FishEyeStereoCalibration::setCameraModel(int flags) {
    m_flags = flags;
}

bool FishEyeStereoCalibration::calibrateLeftCamera(
    ...
    cv::Mat& K1, cv::Mat& D1)
{
    ...
    for (size_t i = 0; i < imagePoints.size(); ++i) {
        ...
        for (int r = 0; r < boardSize.height; ++r) {
            for (int c = 0; c < boardSize.width; ++c) {
                obj.push_back(cv::Point3f(c * 50.0f, r * 50.0f, 0.0f));
            }
        }
    ...
    double rms = cv::fisheye::calibrate(objectPoints, imagePoints, imageSize, K1, D1,
                                        rvecs, tvecs, m_flags);
...
bool FishEyeStereoCalibration::calibrateRightCamera(
    const std::vector<std::string>& imagePaths,
    cv::Size boardSize,
    cv::Mat& K2, cv::Mat& D2)
{
    ...
    double rms = cv::fisheye::calibrate(objectPoints, imagePoints, imageSize, K2, D2,
                                        rvecs, tvecs, m_flags);
...
bool FishEyeStereoCalibration::calibrateStereoCamera(
    ...
    StereoFishEyeCalibration& calibration)
{
    ...
    for (size_t i = 0; i < leftImagePaths.size(); ++i) {
        ...
        bool found1 = findChessboardCorners(img1, boardSize, corners1,
            cv::CALIB_CB_ADAPTIVE_THRESH | cv::CALIB_CB_NORMALIZE_IMAGE);
        bool found2 = findChessboardCorners(img2, boardSize, corners2,
            cv::CALIB_CB_ADAPTIVE_THRESH | cv::CALIB_CB_NORMALIZE_IMAGE);
    ...
    calibration.rms = cv::fisheye::stereoCalibrate(objectPoints, imagePoints1, imagePoints2,
                                                     ...
                                                     flags, cv::TermCriteria(3, 30, 1e-6));
...
bool FishEyeStereoCalibration::stereoRectify(
    StereoFishEyeCalibration& calibration,
...
void FishEyeStereoCalibration::computeRectificationMaps(
    StereoFishEyeCalibration& calibration)
{
    cv::fisheye::initUndistortRectifyMap(calibration.K1, calibration.D1, calibration.R1,
                                          calibration.P1, m_newImageSize, CV_16SC2,
                                          m_map1l, m_map2l);
    cv::fisheye::initUndistortRectifyMap(calibration.K2, calibration.D2, calibration.R2,
                                          calibration.P2, m_newImageSize, CV_16SC2,
                                          m_map1r, m_map2r);
}

void FishEyeStereoCalibration::undistortImage(
    ...
    bool useOptimalProjection)
{
    ...
    if (useOptimalProjection) {
        cv::Mat K_new = cv::Mat::eye(3, 3, CV_64F);
        cv::fisheye::estimateNewCameraMatrixForUndistortRectify(K, D, outputSize,
            cv::Mat::eye(3, 3, CV_64F), K_new, 0.0, outputSize);
        cv::fisheye::undistortImage(input, output, K, D, K_new, outputSize);
    } else {
        cv::Mat map1, map2;
...
void FishEyeStereoCalibration::stereoUndistortRectify(
    ...
    const StereoFishEyeCalibration& calibration)
{
    if (!m_rectifyInitialized) {
        StereoFishEyeCalibration calibCopy = calibration;
...
void FishEyeStereoCalibration::saveCalibration(
...
bool FishEyeStereoCalibration::loadCalibration(
    const std::string& filename,
    StereoFishEyeCalibration& calibration)
{
...
std::vector<cv::Point2f> FishEyeStereoCalibration::findFishEyeChessboardCorners(
...
bool FishEyeStereoCalibration::loadImages(
